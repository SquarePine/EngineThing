#ifndef SQUAREPINE_LINE_H
#define SQUAREPINE_LINE_H
...
public:
    ...
    */
    ...
    Line<float> toFloat() const noexcept        { return Line (start.toFloat(), end.toFloat()); }

    /** Casts this line to double coordinates. */
    Line<double> toDouble() const noexcept      { return Line (start.toDouble(), end.toDouble()); }
    ...
    */
    ...
    {
        ValueType delta = end - start;
        ValueType length = juce_hypot ((double) delta.x,
                                  (double) delta.y);
        if (length <= 0)
            return start;

        return Line (start.x + static_cast<ValueType> ((delta.x * distanceFromStart - delta.y * perpendicularDistance) / length),
    ...
    */
    ...
    {
        ValueType delta = end - start;
        ValueType length = delta.x * delta.x + delta.y * delta.y;

        if (length > 0)
        {
            ValueType prop = ((targetPoint.x - start.x) * delta.x + (targetPoint.y - start.y) * delta.y) / (double) length;
        ...
        ValueType fromStart = targetPoint.getDistanceFrom (start);
        ValueType fromEnd = targetPoint.getDistanceFrom (end);
    ...
    /** Finds the point on this line which is nearest to a given point,
        and returns its position as a proportional position along the line.
    ...
    */
    ...
    {
        return Line (getPointAlongLine (std::min (distanceToShortenBy, getLength())), end);
    }
    ...
    */
    Line withShortenedEnd (ValueType distanceToShortenBy) const noexcept
    {
        ...
        return Line (start, getPointAlongLine (length - std::min (distanceToShortenBy, length)));
    }
...
private:
    ...
    static bool findIntersection (Point<ValueType> p1, Point<ValueType> p2,
                                  Point<ValueType> p3, Point<ValueType> p4,
                                  Point<ValueType>& intersection) noexcept
    {
        ...
        ValueType d1 = p2 - p1;
        ValueType d2 = p4 - p3;
        ValueType divisor = d1.x * d2.y - d2.x * d1.y;
        ...
        if (divisor == 0)
        {
            if (! (d1.isOrigin() || d2.isOrigin()))
            {
                if (d1.y == 0 && d2.y != 0)
                {
                    ValueType along = (p1.y - p3.y) / d2.y;
                ...
                if (d2.y == 0 && d1.y != 0)
                {
                    ValueType along = (p3.y - p1.y) / d1.y;
                ...
                if (d1.x == 0 && d2.x != 0)
                {
                    ValueType along = (p1.x - p3.x) / d2.x;
                ...
                if (d2.x == 0 && d1.x != 0)
                {
                    ValueType along = (p3.x - p1.x) / d1.x;
        ...
        ValueType along1 = ((p1.y - p3.y) * d2.x - (p1.x - p3.x) * d2.y) / divisor;
        intersection = p1 + d1 * along1;

        if (! isZeroToOne (along1))
            return false;

        ValueType along2 = ((p1.y - p3.y) * d1.x - (p1.x - p3.x) * d1.y) / divisor;
        return isZeroToOne (along2);
    }
};

#endif //SQUAREPINE_LINE_H
