...
Colour::Colour (PixelARGB pixels) noexcept :
    argb (pixels)
{
}

Colour::Colour (PixelRGB pixels) noexcept :
    argb (Colour (pixels.getInARGBMaskOrder()).argb)
{
}
...
Colour Colour::withMultipliedAlpha (float alphaMultiplier) const noexcept
{
    ...
    PixelARGB newCol (argb);
    newCol.setAlpha ((uint8) std::min (0xff, roundToInt (alphaMultiplier * newCol.getAlpha())));
    return Colour (newCol);
}
...
{
    ColourHelpers::HSB hsb (*this);
    hsb.saturation = std::min (1.0f, hsb.saturation * amount);
    return hsb.toColour (*this);
}
...
{
    ColourHelpers::HSB hsb (*this);
    hsb.brightness = std::min (1.0f, hsb.brightness * amount);
    return hsb.toColour (*this);
}
...
{
   return overlaidWith ((getPerceivedBrightness() >= 0.5f ? Colours::black : Colours::white).withAlpha (amount));
}
...
Colour Colour::contrasting (Colour target, float minContrast) const noexcept
{
    ...
    const float y1 = std::max (0.0f, bg.y - minContrast);
    const float y2 = std::min (1.0f, bg.y + minContrast);
    fg.y = (std::abs (y1 - bg.y) > std::abs (y2 - bg.y)) ? y1 : y2;

    return fg.toColour();
}

Colour Colour::contrasting (Colour colour1, Colour colour2) noexcept
{
    ...
    for (float i = 0.0f; i < 1.0f; i += 0.02f)
    {
        ...
        const float dist = std::min (d1, std::min (d2, std::min (1.0f - d1, 1.0f - d2)));
