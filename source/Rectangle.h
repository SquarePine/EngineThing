#ifndef SQUAREPINE_RECTANGLE_H
#define SQUAREPINE_RECTANGLE_H
...
public:
    ...
    */
    ...
    Rectangle (Point<ValueType> corner1, Point<ValueType> corner2) noexcept :
        pos (std::min (corner1.x, corner2.x), std::min (corner1.y, corner2.y)),
    ...
    */
    ...
    {
        return Point<ValueType> (pos.x + w / (ValueType) 2, pos.y + h / (ValueType) 2);
    }
    ...
    */
    ...
    inline void setCentre (ValueType newCentreX, ValueType newCentreY) noexcept
    {
    ...
    /** Moves the x position, adjusting the width so that the right-hand edge remains in the same place.
        ...
        @see withLeft
    */
    void setLeft (ValueType newLeft) noexcept                   { w = std::max (ValueType(), pos.x + w - newLeft); pos.x = newLeft; }
    ...
    /** Returns a new rectangle with a different x position, but the same right-hand edge as this one.
        ...
        @see setLeft
    */
    Rectangle withLeft (ValueType newLeft) const noexcept       { return Rectangle (newLeft, pos.y, std::max (ValueType(), pos.x + w - newLeft), h); }

    /** Moves the y position, adjusting the height so that the bottom edge remains in the same place.
        If the y is moved to be below the current bottom edge, the height will be set to zero.
        @see withTop
    */
    void setTop (ValueType newTop) noexcept                     { h = std::max (ValueType(), pos.y + h - newTop); pos.y = newTop; }
    ...
    /** Returns a new rectangle with a different y position, but the same bottom edge as this one.
        ...
        @see setTop
    */
    Rectangle withTop (ValueType newTop) const noexcept         { return Rectangle (pos.x, newTop, w, std::max (ValueType(), pos.y + h - newTop)); }
    ...
    */
    void setRight (ValueType newRight) noexcept                 { pos.x = std::min (pos.x, newRight); w = newRight - pos.x; }
    ...
    /** Returns a new rectangle with a different right-hand edge position, but the same left-hand edge as this one.
        ...
        @see setRight
    */
    Rectangle withRight (ValueType newRight) const noexcept     { return Rectangle (std::min (pos.x, newRight), pos.y, std::max (ValueType(), newRight - pos.x), h); }

    /** Adjusts the height so that the bottom edge of the rectangle has this new value.
        If the new bottom is lower than the current Y value, the Y will be pushed down to match it.
        @see getBottom, withBottom
    */
    void setBottom (ValueType newBottom) noexcept               { pos.y = std::min (pos.y, newBottom); h = newBottom - pos.y; }

    /** Returns a new rectangle with a different bottom edge position, but the same top edge as this one.
        If the new y is beyond the bottom of the current rectangle, the height will be set to zero.
        @see setBottom
    */
    Rectangle withBottom (ValueType newBottom) const noexcept   { return Rectangle (pos.x, std::min (pos.y, newBottom), w, std::max (ValueType(), newBottom - pos.y)); }
    ...
    */
    ...
    {
        ValueType nw = std::max (ValueType(), w + deltaX * 2);
        ValueType nh = std::max (ValueType(), h + deltaY * 2);
    ...
    */
    ...
    {
        const Rectangle r (pos.x, pos.y, w, std::min (amountToRemove, h));
        pos.y += r.h; h -= r.h;
        return r;
    }
    ...
    */
    ...
    {
        const Rectangle r (pos.x, pos.y, std::min (amountToRemove, w), h);
        pos.x += r.w; w -= r.w;
        return r;
    }
    ...
    */
    ...
    {
        return Rectangle (jlimit (pos.x, pos.x + w, point.x),
    ...
    */
    ...
    {
        return contains (line.getStart())
            || contains (line.getEnd())
    ...
    */
    ...
    {
        ValueType nx = std::max (pos.x, other.pos.x);
        ValueType ny = std::max (pos.y, other.pos.y);
        ValueType nw = std::min (pos.x + w, other.pos.x + other.w) - nx;

        if (nw >= ValueType())
        {
            ValueType nh = std::min (pos.y + h, other.pos.y + other.h) - ny;
    ...
    */
    ...
    {
        ValueType maxX = std::max (otherX, pos.x);
        otherW = std::min (otherX + otherW, pos.x + w) - maxX;

        if (otherW > ValueType())
        {
            ValueType maxY = std::max (otherY, pos.y);
    ...
    */
    ...
    {
        return intersectRectangle (rectangleToClip.pos.x, rectangleToClip.pos.y, rectangleToClip.w, rectangleToClip.h);
    }
    ...
    */
    Rectangle getUnion (Rectangle other) const noexcept
    {
        ...
        ValueType newX = std::min (pos.x, other.pos.x);
        ValueType newY = std::min (pos.y, other.pos.y);
    ...
    */
    bool reduceIfPartlyContainedIn (Rectangle other) noexcept
    {
        ...
        ValueType otherR = other.getRight();
        if (pos.x >= other.pos.x && pos.x < otherR) inside = 1;

        ValueType otherB = other.getBottom();
    ...
    */
    ...
    {
        ValueType newW = std::min (w, areaToFitWithin.getWidth());
        ValueType newH = std::min (h, areaToFitWithin.getHeight());
    ...
    */
    Rectangle transformedBy (const AffineTransform& transform) const noexcept
    {
        ...
        ValueType x1 = static_cast<FloatType> (pos.x),     y1 = static_cast<FloatType> (pos.y);
        ValueType x2 = static_cast<FloatType> (pos.x + w), y2 = static_cast<FloatType> (pos.y);
        ValueType x3 = static_cast<FloatType> (pos.x),     y3 = static_cast<FloatType> (pos.y + h);
        ValueType x4 = static_cast<FloatType> (x2),        y4 = static_cast<FloatType> (y3);

        transform.transformPoints (x1, y1, x2, y2);
        transform.transformPoints (x3, y3, x4, y4);

        ValueType rx1 = std::min (x1, x2, x3, x4);
        ValueType rx2 = std::max (x1, x2, x3, x4);
        ValueType ry1 = std::min (y1, y2, y3, y4);
        ValueType ry2 = std::max (y1, y2, y3, y4);
    ...
    */
    ...
    {
        ValueType x = std::max (x1, x2);
        w1 = std::min (x1 + w1, x2 + w2) - x;

        if (w1 > ValueType())
        {
            ValueType y = std::max (y1, y2);
